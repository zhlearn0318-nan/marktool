...
def _limited_app(max_bytes: int) -> FastAPI:
    ...
    @app.post("/api/v1/metadata-repair-plans")
    async def consume_repair(request: Request):
        return {"size": len(await request.body())}
...
@pytest.mark.parametrize(
    "path", ["/api/v1/metadata-label-jobs", "/api/v1/metadata-repair-plans"]
)
def test_chunked_body_without_content_length_is_counted(path):
